    ...
    public class AddMoveOrderCommandHandler(IMoveOrderRepository moveOrderRepository, IUserRepository userRepository, IProductRepository productRepository, IWarehouseReceivingRepository warehouseReceivingRepository) : IRequestHandler<AddMoveOrderCommand, Result<object>>
    {
        ...
        public async Task<Result<object>> Handle(AddMoveOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var product in request.AddMoveOrderDto.AddMoveOrderProducts)
            {
                ...
                if (duplicateCount > 1)
                {
                    return Result<object>.Failure($"Duplicate product found", HttpStatusCode.BadRequest);
                }
            }

            var existingProducts = await _productRepository.AllExistsAsync([.. request.AddMoveOrderDto.AddMoveOrderProducts.Select(x => x.ProductId)], cancellationToken);

            if (!existingProducts)
            {
                return Result<object>.Failure("One or more products do not exist", HttpStatusCode.BadRequest);
            }
            ...
            {
                var hasAvailableReserve = await _warehouseReceivingRepository.ProductHasAvailableReserve(request.AddMoveOrderDto.WarehouseId, product.Id, product.Quantity, cancellationToken);

                if (!hasAvailableReserve)
                {
                    return Result<object>.Failure($"Not enough reserve for product {product.ItemCode}", HttpStatusCode.BadRequest);
                }
            }
            ...
            foreach (var product in consolidatedProducts)
            {
                ...
                var affectedWarehouseReceivings = await _warehouseReceivingRepository.GetProductAffectedWarehouseReceivings(request.AddMoveOrderDto.WarehouseId, product.Id, product.Quantity, cancellationToken);
