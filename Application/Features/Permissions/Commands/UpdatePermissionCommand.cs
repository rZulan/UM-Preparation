...
public record UpdatePermissionCommand(int? UserId, int Id, UpdatePermissionDto UpdatePermissionDTO)
    : IRequest<Result<object>>;

public class UpdatePermissionCommandHandler(IPermissionRepository permissionRepository, IUserRepository userRepository)
    : IRequestHandler<UpdatePermissionCommand, Result<object>>
{
    ...
    public async Task<Result<object>> Handle(UpdatePermissionCommand request, CancellationToken cancellationToken)
    {
        ...
        if (existingPermission == null) return Result<object>.Failure("Permission not found", HttpStatusCode.NotFound);

        if (!string.IsNullOrEmpty(request.UpdatePermissionDTO.Name))
        {
            var existingName =
                await _permissionRepository.AnyDuplicateAsync(request.Id, request.UpdatePermissionDTO.Name,
                    cancellationToken);

            if (existingName) return Result<object>.Failure("Permission name already exists");
