    ...
    : IRequestHandler<ToggleRoleActiveCommand, Result<object>>
{
    ...
    public async Task<Result<object>> Handle(ToggleRoleActiveCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.IsActive && existingRole.IsActive) return Result<object>.Failure("Role is already active");

        if (!request.IsActive && !existingRole.IsActive) return Result<object>.Failure("Role is already archived");
