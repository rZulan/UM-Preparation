    ...
    : IRequestHandler<ToggleUomActiveCommand, Result<object>>
{
    ...
    public async Task<Result<object>> Handle(ToggleUomActiveCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.IsActive && existingUom.IsActive) return Result<object>.Failure("Uom is already active");

        if (!request.IsActive && !existingUom.IsActive) return Result<object>.Failure("Uom is already archived");
