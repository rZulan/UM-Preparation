    ...
    : IRequestHandler<ToggleUserActiveCommand, Result<object>>
{
    public async Task<Result<object>> Handle(ToggleUserActiveCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.Toggle && existingUser.IsActive) return Result<object>.Failure("User is already active");

        if (!request.Toggle && !existingUser.IsActive) return Result<object>.Failure("User is already archived");
