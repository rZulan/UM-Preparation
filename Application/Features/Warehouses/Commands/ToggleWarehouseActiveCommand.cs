...
namespace Application.Features.Warehouses.Commands
{
    ...
    public class ToggleWarehouseActiveCommandHandler(IWarehouseRepository warehouseRepository, IUserRepository userRepository) : IRequestHandler<ToggleWarehouseActiveCommand, Result<object>>
    {
        ...
        public async Task<Result<object>> Handle(ToggleWarehouseActiveCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return Result<object>.Failure("Warehouse not found");
            }
