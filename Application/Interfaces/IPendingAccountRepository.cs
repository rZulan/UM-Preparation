...
public interface IPendingAccountRepository
{
    ...
    Task<List<PendingAccount>> GetAllAsync(GenericFiltersDto genericFiltersDTO, Sort sort,
    ...
    /// <summary>
    ///     Returns a pending account matching the given employee prefix and ID number, or <see langword="null" /> if not
    ///     found.
    /// </summary>
    ...
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    Task<PendingAccount?> GetByFullIdNoAsync(string employeePrefix, string employeeId,
        CancellationToken cancellationToken);
